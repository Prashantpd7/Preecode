...
const BuiltResume = require('./builtResumeModel');
...
async function getBuiltResumes(req, res) {
  try {
    const resumes = await BuiltResume.find({ userId: req.user._id }).sort({ updatedAt: -1 });
    res.json(resumes);
  } catch (err) {
